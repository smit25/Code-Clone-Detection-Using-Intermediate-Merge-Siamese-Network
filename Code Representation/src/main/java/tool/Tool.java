...
public class Tool {
    ...
    /**
     * CMD corresponding instruction and record log
     */
     ...
     * Can be written as a dot file format id, parsing it
     */
    ...
    /**
     * According to the structure node (simplified version of the node CFG) and pure edge (structure simple edge between node) to generate the CFG dot files
     */
    ...
    /**
     * To maintain consistent, method to construct for CFG Node
     */
    ...
    /**
     * Method from the method Return to build for CFG node
     */
    ...
    /**
     * Whether to join the edge has been in existence
     */
    ...
    /**
     * Construction Method of CFG graph, save as dot files
     */
    ...
    /**
     * By CFG nodes and edges of constructor CFG figure, does not contain the Identifier and the Literal and so on, only use the structure of the node
     * Among them, the structure node includes: Method; The Call; The Return; Method the Return
     */
    public static CFGGraph constructCFGGraphByNodeAndEdge(Set<CFGNode> cfgNodeSet, Set<CFGEdge> cfgEdgeSet, Map<String, CFGNode> cfgNodeId2CfgNode) {
        ...
        for (CFGEdge edge : cfgEdgeSet) {
            ...
            // Do not belong to the same structure node && did not exist
    ...
    /**
     * From the source Folder Path, with all the source files
     */
    ...
    /**
     * When the folder does not exist, create
     */
...
//        cfgNodeSet.forEach(System.out::println);
    ...
    private static void inOrderAST(ASTNode root, StringBuilder res, Map<ASTNode, List<ASTNode>> node2Children) {
        if (root != null) {
            List<ASTNode> children = node2Children.get(root);
            if (children != null) {
                for (ASTNode astNode : children) {
                    inOrderAST(astNode, res, node2Children);
                }
            }
            res.append(root.getPropertyMap().getOrDefault(CFGConfig.CODE_PROPERTY, "")).append(" ");
        }
    ...
    public static String traverseAST(AST ast) {
        ...
        inOrderAST(root, sb, node2Children);
